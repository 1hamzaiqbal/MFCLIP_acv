...
import yaml
# sys.path.append('../../')
from loss.head.AdaCos import AdaCos
from loss.head.AdaM_Softmax import Adam_Softmax
from loss.head.AM_Softmax import AM_Softmax
from loss.head.ArcFace import ArcFace
from loss.head.CircleLoss import CircleLoss
from loss.head.CurricularFace import CurricularFace
from loss.head.MV_Softmax import MV_Softmax
from loss.head.NPCFace import NPCFace
...
class HeadFactory:
    ...
    """
    def __init__(self, head_type, config, num_classes=None, head_conf_file='loss/head/head_conf.yaml'):
        self.head_type = head_type
        self.num_classes = num_classes
        with open(head_conf_file) as f:
            head_conf = yaml.load(f, Loader=yaml.FullLoader)
            self.head_param = head_conf[head_type]
        self.head_param['num_class'] = config['num_classes']
        self.head_param['feat_dim'] = config['output_dim']
        print('head param:')
